...
import { CozeBot, uuid } from './coze.js'
...
export class PoemPalette {
  ...
  mjToken: string
  cozeToken: string
  botId: string = '7366137814322724873'
  mjEndpoint: string = 'https://api.opengptgod.com'
  ...
  constructor (
    ...
    this.mjToken = mjToken
    this.cozeToken = cozeToken
    this.botId = botId || this.botId
    this.cozeEndpoint = cozeEndpoint || this.cozeEndpoint
    this.mjEndpoint = mjEndpoint || this.mjEndpoint
  }

  async chat (user: string, query: string, chatHistory: Chat[] = [], conversation_id?:string): Promise<any> {
    ...
    const reg = /{([^}]*)}/
    const result = reg.exec(str as string)
    ...
    try {
      ...
      return await this.chat(user, query, chatHistory)
    }
  };
  ...
  async createImage (textPoem: any, disc:string = '') {
    ...
    const headers = {
      ...
      Authorization: 'Bearer ' + this.mjToken,
    }
    const data = {
      model: 'mj-chat',
      messages: [
    ...
    /* stable-diffusion
  {
    ...
    */
    ...
    try {
      const res = await axios.post(this.mjEndpoint, data, { headers })
      console.info('res:', JSON.stringify(res.data))
